...
for(i in 1:K){
  ...
  for(j in 2:160){
    ...
    # Random Forest
    #for(tree in vectorTree) {
     # rf.forward <- randomForest(factor(y.app[folds!=i])~., data=as.data.frame(X.forward.data[folds!=i,1:j]), xtest=as.data.frame(X.forward.data[folds==i,1:j]), ytest=factor(y.app[folds==i]), ntree=tree)
